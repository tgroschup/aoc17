...
class ExecuteDay4 extends FunSuite {
    ...
    test("Solution one") {
        val result = new Passphrase(input).countValid
        assertResult(383)(result)
        println("First solution of day four: " + result)
    }
    ...
    test("Solution two") {
        val result = new Passphrase(input).countValidWithAnagrams
        assertResult(265)(result)
        println("Second solution for day four is: " + result)
    }
}
